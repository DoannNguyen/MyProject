  ...
  const [dataDay, setDataDay] = useState()
       ...
       try {
        ...
        await fetch(
          ...
          .then(data =>
           {
             setDataHour(
              data[0].hour.filter(item => item.time.substring(11, 13) >= new Date().getHours() ),
            )
            setDataDay(data)
           }
    ...
    const RenderItemHour = ({item}) => (
      <View style={styles.viewItem}>
        <Text style ={{color: '#fff', fontSize: 15}}>{item.temp_c.toFixed()}°</Text>
        <Image
          source={{uri: `https:${item.condition.icon}`}}
          style={{width: 75, height: 75}}
        />
        <Text style = {{color: '#fff', fontSize: 15}}>{item.time.substring(11, 16)}</Text>
      </View>
    );
        ...
        <SafeAreaView>
          <ScrollView>
            ...
            <ScrollView horizontal={true}>
              <FlatList
                data={dataDay}
                renderItem={({item}) => <RenderItemDay item={item} />}
